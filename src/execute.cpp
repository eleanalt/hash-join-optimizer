...
struct JoinAlgorithm {
    ...
    auto run() {
        ...
        //unchained hash choice
    #ifdef USE_UNCHAINED_HASH
        UnchainedHash<T, size_t> hash_table;

        if (build_left) {
            // BUILD from left
            hash_table.reserve(static_cast<uint32_t>(left.size()));
            for (auto&& [idx, record] : left | views::enumerate) {
                std::visit(
                    [&hash_table, idx = static_cast<size_t>(idx)](const auto& key) {
            ...
            // BUILD from right
            hash_table.reserve(static_cast<uint32_t>(right.size()));
